% left hemisphere's Ring-positive ROI
clear;
close all;

load('roi_Data.mat', 'lrp_roi');

% threshold for discriminating the biased voxels
t_thr=1;    % t-statistics threshold
B_thr=0.5;    % beta coefficients
B_lim=5;
...
j=1; 
for i=1:numel(lrp_roi(:,1))
    if (lrp_roi(i, 11)+lrp_roi(i, 14)) - (lrp_roi(i, 23)+lrp_roi(i, 26)) > 2*B_thr ...
            && (abs(lrp_roi(i, 5))<B_lim && abs(lrp_roi(i, 8))<B_lim && abs(lrp_roi(i, 17))<B_lim && abs(lrp_roi(i, 20))<B_lim) ...
            && (abs(lrp_roi(i, 11))<B_lim && abs(lrp_roi(i, 14))<B_lim && abs(lrp_roi(i, 23))<B_lim && abs(lrp_roi(i, 26))<B_lim) ...
...
j=1; 
for i=1:numel(lrp_roi(:,1))
    if (lrp_roi(i, 23)+lrp_roi(i, 26)) - (lrp_roi(i, 11)+lrp_roi(i, 14)) > 2*B_thr ... 
            && (abs(lrp_roi(i, 5))<B_lim && abs(lrp_roi(i, 8))<B_lim && abs(lrp_roi(i, 17))<B_lim && abs(lrp_roi(i, 20))<B_lim) ...
            &&  (abs(lrp_roi(i, 11))<B_lim && abs(lrp_roi(i, 14))<B_lim && abs(lrp_roi(i, 23))<B_lim && abs(lrp_roi(i, 26))<B_lim) ...
%             && (lrn_roi(i, 6)>t_thr || lrn_roi(i, 9)>t_thr || lrn_roi(i, 18)>t_thr || lrn_roi(i, 21)>t_thr) ...
%             && (lrn_roi(i, 12)<-t_thr || lrn_roi(i, 15)<-t_thr || lrn_roi(i, 24)<-t_thr || lrn_roi(i, 27)<-t_thr) %((ldn_roi(i, 6)+ldn_roi(i, 9))> 2*t_thr || (ldn_roi(i, 18)+ldn_roi(i, 21)> 2*t_thr))
        reye_bias_ldn(j, :)=lrp_roi(i, :);
        j=j+1;
    end
end
...
j=1; 
for i=1:numel(lrp_roi(:,1))
    if (lrp_roi(i, 11)+lrp_roi(i, 23)) - (lrp_roi(i, 14)+lrp_roi(i, 26)) > 2*B_thr ...
            && (abs(lrp_roi(i, 5))<B_lim && abs(lrp_roi(i, 8))<B_lim && abs(lrp_roi(i, 17))<B_lim && abs(lrp_roi(i, 20))<B_lim) ...
            &&  (abs(lrp_roi(i, 11))<B_lim && abs(lrp_roi(i, 14))<B_lim && abs(lrp_roi(i, 23))<B_lim && abs(lrp_roi(i, 26))<B_lim) ...
%             && (lrn_roi(i, 6)>t_thr || lrn_roi(i, 9)>t_thr || lrn_roi(i, 18)>t_thr || lrn_roi(i, 21)>t_thr) ...
...
j=1; 
for i=1:numel(lrp_roi(:,1))
    if (lrp_roi(i, 14)+lrp_roi(i, 26)) - (lrp_roi(i, 11)+lrp_roi(i, 23)) > 2*B_thr ... 
            && (abs(lrp_roi(i, 5))<B_lim && abs(lrp_roi(i, 8))<B_lim && abs(lrp_roi(i, 17))<B_lim && abs(lrp_roi(i, 20))<B_lim) ...
            && (abs(lrp_roi(i, 11))<B_lim && abs(lrp_roi(i, 14))<B_lim && abs(lrp_roi(i, 23))<B_lim && abs(lrp_roi(i, 26))<B_lim) ...
%             && (lrn_roi(i, 6)>t_thr || lrn_roi(i, 9)>t_thr || lrn_roi(i, 18)>t_thr || lrn_roi(i, 21)>t_thr) ...
%             && (lrn_roi(i, 12)<-t_thr || lrn_roi(i, 15)<-t_thr || lrn_roi(i, 24)<-t_thr || lrn_roi(i, 27)<-t_thr) %((ldn_roi(i, 9)+ldn_roi(i, 21))> 2*t_thr || (ldn_roi(i, 6)+ldn_roi(i, 18)> 2*t_thr))
        rori_bias_ldn(j, :)=lrp_roi(i, :);
        j=j+1;
    end
end
