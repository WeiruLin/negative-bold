% right hemisphere's Ring-negative ROI
clear;
close all;

load('roi_Data.mat', 'rrp_roi');
...
j=1; 
for i=1:numel(rrp_roi(:,1))
    if (rrp_roi(i, 11)+rrp_roi(i, 14)) - (rrp_roi(i, 23)+rrp_roi(i, 26)) > 2*B_thr ...
            && (abs(rrp_roi(i, 5))<B_lim && abs(rrp_roi(i, 8))<B_lim && abs(rrp_roi(i, 17))<B_lim && abs(rrp_roi(i, 20))<B_lim) ...
            && (abs(rrp_roi(i, 11))<B_lim && abs(rrp_roi(i, 14))<B_lim && abs(rrp_roi(i, 23))<B_lim && abs(rrp_roi(i, 26))<B_lim) ...
%             && (rdn_roi(i, 6)>t_thr || rdn_roi(i, 9)>t_thr || rdn_roi(i, 18)>t_thr || rdn_roi(i, 21)>t_thr) ... 
%             && (rdn_roi(i, 12)<-t_thr || rdn_roi(i, 15)<-t_thr || rdn_roi(i, 24)<-t_thr || rdn_roi(i, 27)<-t_thr) %((ldn_roi(i, 6)+ldn_roi(i, 9))> 2*t_thr || (ldn_roi(i, 18)+ldn_roi(i, 21)> 2*t_thr))
        leye_bias_ldn(j, :)=rrp_roi(i, :);
        j=j+1;
    end
end
...
suptitle('Right hemisphere: left eye biased');

%%
% extract right eye biased voxels
j=1; 
for i=1:numel(rrp_roi(:,1))
    if (rrp_roi(i, 23)+rrp_roi(i, 26)) - (rrp_roi(i, 11)+rrp_roi(i, 14)) > 2*B_thr ... 
            && (abs(rrp_roi(i, 5))<B_lim && abs(rrp_roi(i, 8))<B_lim && abs(rrp_roi(i, 17))<B_lim && abs(rrp_roi(i, 20))<B_lim) ...
            &&  (abs(rrp_roi(i, 11))<B_lim && abs(rrp_roi(i, 14))<B_lim && abs(rrp_roi(i, 23))<B_lim && abs(rrp_roi(i, 26))<B_lim) ...
%             && (rdn_roi(i, 6)>t_thr || rdn_roi(i, 9)>t_thr || rdn_roi(i, 18)>t_thr || rdn_roi(i, 21)>t_thr) ...
%             && (rdn_roi(i, 12)<-t_thr || rdn_roi(i, 15)<-t_thr || rdn_roi(i, 24)<-t_thr || rdn_roi(i, 27)<-t_thr) %((ldn_roi(i, 6)+ldn_roi(i, 9))> 2*t_thr || (ldn_roi(i, 18)+ldn_roi(i, 21)> 2*t_thr))
        reye_bias_ldn(j, :)=rrp_roi(i, :);
        j=j+1;
    end
end
...
suptitle('Right hemisphere: right eye biased');

%%
% extract left orientation biased voxels
j=1; 
for i=1:numel(rrp_roi(:,1))
    if (rrp_roi(i, 11)+rrp_roi(i, 23)) - (rrp_roi(i, 14)+rrp_roi(i, 26)) > 2*B_thr ...
            && (abs(rrp_roi(i, 5))<B_lim && abs(rrp_roi(i, 8))<B_lim && abs(rrp_roi(i, 17))<B_lim && abs(rrp_roi(i, 20))<B_lim) ...
            &&  (abs(rrp_roi(i, 11))<B_lim && abs(rrp_roi(i, 14))<B_lim && abs(rrp_roi(i, 23))<B_lim && abs(rrp_roi(i, 26))<B_lim) ...
%             && (rdn_roi(i, 6)>t_thr || rdn_roi(i, 9)>t_thr || rdn_roi(i, 18)>t_thr || rdn_roi(i, 21)>t_thr) ...
%             && (rdn_roi(i, 12)<-t_thr || rdn_roi(i, 15)<-t_thr || rdn_roi(i, 24)<-t_thr || rdn_roi(i, 27)<-t_thr) %((ldn_roi(i, 9)+ldn_roi(i, 21))> 2*t_thr || (ldn_roi(i, 6)+ldn_roi(i, 18)> 2*t_thr))
        lori_bias_ldn(j, :)=rrp_roi(i, :);
        j=j+1;
    end
end
...
suptitle('Right hemisphere: left orientation biased');

%%
% extract right orientation biased voxels
j=1; 
for i=1:numel(rrp_roi(:,1))
    if (rrp_roi(i, 14)+rrp_roi(i, 26)) - (rrp_roi(i, 11)+rrp_roi(i, 23)) > 2*B_thr ... 
            && (abs(rrp_roi(i, 5))<B_lim && abs(rrp_roi(i, 8))<B_lim && abs(rrp_roi(i, 17))<B_lim && abs(rrp_roi(i, 20))<B_lim) ...
            && (abs(rrp_roi(i, 11))<B_lim && abs(rrp_roi(i, 14))<B_lim && abs(rrp_roi(i, 23))<B_lim && abs(rrp_roi(i, 26))<B_lim) ...
%             && (rdn_roi(i, 6)>t_thr || rdn_roi(i, 9)>t_thr || rdn_roi(i, 18)>t_thr || rdn_roi(i, 21)>t_thr) ...
%             && (rdn_roi(i, 12)<-t_thr || rdn_roi(i, 15)<-t_thr || rdn_roi(i, 24)<-t_thr || rdn_roi(i, 27)<-t_thr) %((ldn_roi(i, 9)+ldn_roi(i, 21))> 2*t_thr || (ldn_roi(i, 6)+ldn_roi(i, 18)> 2*t_thr))
        rori_bias_ldn(j, :)=rrp_roi(i, :);
        j=j+1;
    end
end
...
suptitle('Right hemisphere: right orientation biased');